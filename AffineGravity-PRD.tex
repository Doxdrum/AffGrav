...
%%, $T^{i,jk}\epsilon_{ijk}=0$, $a^{ij}$ is antisymmetric, $T^{ij}$ is a symmetric tensor and $c^i$ an arbitrary vector.
...
%% To make sure that it is a solution of the equations of motion, we can expand the action in Eq.~\eqref{4dfull} around the field configuration given by
%% \begin{align}
%%   A_\mu &= \delta_\mu^0 A,\\
%%   T^{\mu,\nu\rho} &= \delta^{\mu}_m \delta^{\nu \rho}_{m 0} T\\
%%   \shortintertext{and}
