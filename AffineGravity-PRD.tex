...
\begin{equation} 
\ddot{x}^i+2F\dot{x}^0\dot{x}^i=0\text{,\qquad and }\qquad\ddot{x}^0+E{(\dot{x}^i)^2}+G(\dot{x}^0)^2=0,
\end{equation}
which we can achieve by setting $F=G=0$ and $E\not=0$ since $(\dot{x}^i)^2$ is already second order in the velocities. Thus, looking again at the equations of motion we can find a non trivial solution if we set all coupling constants to zero but $B_3\not{=}0$, $B_4=-3/2 B_3$, $C_1\not{=}0$ and $E_2= 6 B_3$. 

Additionally, we can think of adding {\color{red}no se como poner esto bien} perturbative inhomogeneous sources to the connection field equations and check on
how these fluctuations affect motion. 
For this, let'us study a matter's lagrangian. Its dependence on the affine connection can be almost arbitrary. However, we will presume that it will depend only on the barred metric (referirEcuacion) $${\mathcal L}_{Matter}={\mathcal L}_{Matter}[\bar{g}^{\mu\nu}].$$ 
Thus, an unmoving Matter point particle at the origin of the reference frame will contribute to the equations of motion for the gravitational field through the component $\bar{g}^{00}$ following the symmetries of the matter source 
\begin{dmath}[compact, spread=2pt]\label{mattervariation}
\delta{\mathcal L}_{Matter}=  \Big(- \frac{1}{2} {dg}^{0} _{m n} C1 T {\delta}^{m n} - \frac{1}{2}  {dt}^{0 0 m} C1 i {p}_{m} + \frac{1}{2}  {dt}^{m 0 n} C1 E {\delta}_{m n}\Big)\frac{\partial{\mathcal L}_{Matter}}{\partial \bar{g}^{00}}.
\end{dmath}
