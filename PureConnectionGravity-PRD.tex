...
%--------- Abstract
\begin{abstract}
  We develop a topological theory of gravity with torsion where metric has a dynamical rather than a kinematical origin. This approach towards gravity resembles pre-geometrical approaches in that a fundamental metric is not assumed even though the  Affine connection gives place to a local inertial structure, which reminds us of Mach's principle that assumes that inertial forces, subject to locally Minkowskian metric structures, should have dynamical origin. Additionally a Newtonian like gravitational force is obtained in the perturbative limit of the theory.
...
In this article we study a diffeomorphism invariant ``toy'' model  consisting  solely of an affine connection  (with torsion), and the strict commitment to formulate a power counting renormalizable action. %{\bfseries \color{blue}
We find the condition on the background metric to be exactly zero to be appealing because the use of a background metric such as in teleparallel theories of gravity would in principle break background independence. %} {\color{red} What condition???//Lo que quise decir es que es atractivo que no haya métrica de background (que sea cero da lo mismo) porque, por ejemplo, es tipico regularizar los operadores de una teoría cualquiera  haciendo un smearing usando una metrica de fondo. Pero en una teoria invariante de difeomorfismos, este tipo de regularizacion debe ser invariante de la metrica de fondo porque sino es practicamente imposible saber que depende de la escogencia de coordenadas y que no. \hfill{}} 
...
\end{align}
where all possible permutations of $n$ elements $\pi\in\mathrm{Z}_n$ have been included in the sums with  different constants $C_\pi$ and $D_\pi$ for  permutation. %In particular, $\pi(i)\in\mathrm{Z}_n$ if $i\in\mathrm Z_n$ represents a permutation of the integer numbers between $1$ and $n$.
...
%%   \end{split}
%% \end{equation}
where the coefficients $B_i$ are related with the original coefficients $C_i$,$D_i$ and $E_i$, and the additional $B_8$ term can be added in 3 dimensions leaving the action invariant under diffeomorphisms. %Moreover, the general curvature can be decomposed into the Riemann curvature plus extra contributions, through the use of the connection decomposition %\footnote{The symbol $\hat{\Gamma}^\lambda{}_{\mu\rho}$ represents the Levi-\v{C}vita connection.}
...
\end{equation}
%% The Bianchi identity, expressed  as $\epsilon^{\mu\nu\lambda}\hat{R}_{\mu\nu}{}^\rho{}_\lambda=0$, leads  to
%% \begin{equation}
%%   \label{bianchi}
