...
\title{Purely affine gravity (Mach)}
...
\end{dmath*}
    {\color{red}podemos quitarle las barras a g y quitamos la segunda linea :P si quieres pero asi es como lo voy a calcular porque no hay que des-densitizar la metrica inversa}
    In $d=4$, (\ref{4dMetric}) could be argued to be the metric of the manifold if  $\Gamma^\lambda{}_{\mu\nu}=\bar{\Gamma}^\lambda{}_{\mu\nu}+{\mathcal K}^\lambda{}_{\mu\nu}$, where ${\mathcal K}^\lambda{}_{\mu\nu}$ is the Contorsion tensor, or equivalently in our variables
