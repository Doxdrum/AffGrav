...
\begin{widetext}
  ...
  \begin{equation}
    ...
    \end{split}
  \end{equation}
  where all possible permutations of $n$ elements $\pi\in\mathrm{Z}_n$ have been included in the sums with  different constants $C_\pi$, $D_\pi$ and $E_\pi$ for  permutation. %In particular, $\pi(i)\in\mathrm{Z}_n$ if $i\in\mathrm Z_n$ represents a permutation of the integer numbers between $1$ and $n$.
\end{widetext}
...
%%   \end{split}
%% \end{equation}
where the coefficients $B_i$ are related with the original coefficients $C_i$, $D_i$ and $E_i$, and the additional $B_9$ term can be added in three dimensions leaving the action invariant under diffeomorphisms. %Moreover, the general curvature can be decomposed into the Riemann curvature plus extra contributions, through the use of the connection decomposition %\footnote{The symbol $\hat{\Gamma}^\lambda{}_{\mu\rho}$ represents the Levi-\v{C}vita connection.}
